    ...
    node {
        ...
        withEnv(["PATH+MAVEN=${tool 'm3'}/bin"]) {
            if(FULL_BUILD) {
                ...
                sh "mvn -B versions:set -DnewVersion=${pom.version}-${BUILD_NUMBER} -f backend/pom.xml"
                sh "mvn -B -Dmaven.test.skip=true clean package -f backend/pom.xml"
                stash name: "artifact", includes: "target/backend-*.war"
            }
        }
    }
}
...
if(FULL_BUILD) {
    stage('Unit Tests') {
        node {
            withEnv(["PATH+MAVEN=${tool 'm3'}/bin"]) {
                sh "mvn -B clean test -f backend/pom.xml"
...
if(FULL_BUILD) {
    stage('Integration Tests') {
        node {
            withEnv(["PATH+MAVEN=${tool 'm3'}/bin"]) {
                sh "mvn -B clean verify -Dsurefire.skip=true -f backend/pom.xml"
        ...
        node {
            withEnv(["PATH+MAVEN=${tool 'm3'}/bin"]) {
                withSonarQubeEnv('sonar'){
                    unstash 'it_tests'
                    unstash 'unit_tests'
                    sh 'mvn sonar:sonar -DskipTests -f backend/pom.xml'
                }
            }
        }
    }
...
if(FULL_BUILD) {
    stage('Artifact Upload') {
        node {
            ...
            def jar = "backend/target/${file}.war"

            sh "cp backend/pom.xml ${file}.pom"
